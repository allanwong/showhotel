...
namespace PettiInn.DAL.Manager.EF5
{
    public class ObjectContextFactory
    {
        public static ObjectContext GetCurrentObjectContext()
        {
            ...
            var objectContext = CallContext.GetData(typeof(ObjectContextFactory).FullName) as ObjectContext;
            if (objectContext == null)
            {
                //如果CallContext数据槽中没有EF上下文，则创建EF上下文，并保存到CallContext数据槽中
                objectContext = new PettiInn.DAL.Entities.EF5.Entities().ObjectContext;//当数据库替换为MySql等，只要在次出EF更换上下文即可。
